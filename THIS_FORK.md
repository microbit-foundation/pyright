...
The translations live in Crowdin alongside the Python Editor's strings and are moved with [`@microbit/i18n-tools`](https://github.com/microbit-foundation/ui/tree/main/packages/i18n-tools), configured in [i18n.config.mjs](./i18n.config.mjs) and run without a local install: `npx --yes --package @microbit/i18n-tools microbit-i18n download` (or `upload` to replace the English `simplified.nls.en-us.json`, `status` for progress) with a Crowdin personal access token in `CROWDIN_PERSONAL_TOKEN`. The translations-download workflow does the download weekly and opens a pull request; the translations-upload workflow runs the upload from the Actions tab. The Python Editor vendors the built worker with its `bin/update-pyright.sh`, so a merged sync reaches users when the editor next rebuilds it.
